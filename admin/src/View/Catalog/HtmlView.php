...
namespace Accountingroot\Component\Accounting\Administrator\View\Catalog;
...
class HtmlView extends BaseHtmlView
{
	...
	protected function addToolbar()
	{
		...
		if ($user->authorise('core.edit.state', 'com_accounting'))
		{
			...
			$childBar->publish('catalog.publish')->listCheck(true);

			$childBar->unpublish('catalog.unpublish')->listCheck(true);

			$childBar->archive('catalog.archive')->listCheck(true);

			if ($this->state->get('filter.published') != -2)
			{
				$childBar->trash('catalog.trash')->listCheck(true);
			}
		}

		if ($this->state->get('filter.published') == -2 && $user->authorise('core.delete', 'com_accounting'))
		{
			$toolbar->delete('catalog.delete')
		...
		if ($tmpl !== 'component')
		{
			ToolbarHelper::help('catalog', true);
		}
	}
}
